...
class PAI(object):
    def __init__(self, username, passwd, url='192.168.193.1', worker=4):
        ...
        self.hdfs = pyhdfs.HdfsClient(
            hosts='{}:50070'.format(self.url), user_name=self.username)

    @staticmethod
    def handle_error(info):
        print("[X] Error: {}".format(info))
        exit()
    ...
    def get_token(self):
        ...
        response = json.loads(requests.post(
            'http://{}/rest-server/api/v1/token'.format(self.url), json=data).text)

        if 'token' not in response:
            self.handle_error(response['message'])
        else:
    ...
    def submit(self, config_path='./pai_configuration.json', dir_path='./', dest_dir='/Users/',
        ...
        if not os.path.exists(config_path):
            self.handle_error(
                "Can not find the configuration of the job. Please run generate_json() \
                    to make the default job configuration!")
        else:

            with open(config_path, 'r') as reader:
                config = json.load(
                    reader, object_pairs_hook=collections.OrderedDict)
                config['jobName'] = self.create_jobname(config['jobName'])
                self.write_configuration(config_path, config)
            job_dest_path = self._process_path(os.path.join(
                dest_dir, self.username, self.username + '~' + config['jobName']))
            self.upload_(dir_path, job_dest_path, exclude_file, exclude_dir)

            headers = {"Content-Type": "application/json",
                       "Authorization": "Bearer {}".format(self.token)}
            response = json.loads(requests.post('http://{}/rest-server/api/v1/user/{}/jobs'.format(
                self.url, self.username), json=config, headers=headers).text)

            if 'code' in response:
                self.handle_error(response['message'])
            else:
                print("=> Upload Job Name: {} Successfully!".format(
                    config['jobName']))

    @staticmethod
    def create_jobname(old_name):
        new_name = ''.join(old_name.split(
            '_')[:-1]) + '_' + str(uuid.uuid4().fields[-1])[:8]
        return new_name
    ...
    def upload_list(self, upload_func, hdfs, file_list, dest_dir):
        try:
            ...
            self.handle_error("Upload failed! \n{}".format(str(e)))

    def upload_(self, dir_path, dest_dir, exclude_file, exclude_dir):
            ...
            for i in range(self.worker):
                t = threading.Thread(target=self.upload_list,
                                     args=(self.upload_func, self.hdfs,
                                           file_list[int(
                                               i/self.worker*file_count):int((i+1)/self.worker*file_count)],
                                           dest_dir,))
                threads.append(t)
